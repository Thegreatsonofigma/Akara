...
flowchart LR
  ...
  A --> C["Salted commitments and Merkle batches"]
  C --> ST["Stellar public ledger: Merkle root only"]
  AD --> DB
  AD --> S
...
| Stellar integrity anchoring, market commitments, quote locks, routes, and verifiable reputation | Detect later alteration of completed trades, disputes, peer-market snapshots, accepted terms, route plans, and reputation records, and provide independently timestamped commitments. | Verified users who search, accept, complete, or dispute trades. | Private Akara snapshot: opaque HMAC subjects, trade-state metadata, receipt digest, dispute-text digest, aggregate reputation metrics, corridor aggregates, accepted terms, route legs, random salt. Public Stellar record: one Merkle root, dedicated Akara public account, transaction hash, ledger time. | Existing Akara listing, trade, evidence, dispute, market, route, and reputation records. | Contract and legitimate interests in service delivery, fraud prevention, integrity, accountability, market transparency, and dispute evidence. Confirm in the final DPIA and privacy notice. | Akara server, Supabase, Stellar public network, Horizon endpoint. | Public ledger receives only the batched Merkle root; authorised admins can inspect the private source snapshot. A user may share a privacy-safe reputation credential voluntarily. | Public ledger commitment is permanent. Private snapshots follow their source trade, market, route, or reputation retention rule. Trust credentials expire after 30 days unless revoked sooner. | Yes. Stellar validators and data services may operate globally. | No direct identifiers or internal UUIDs on-chain, salted commitments, HMAC subjects, Merkle batching, credential data minimisation, dedicated low-balance signer, fee ceiling, HTTPS, append-only database triggers, testnet-first activation. | Engineering / Privacy Lead. | Obtain DPCO or privacy-counsel approval of lawful basis, credential sharing copy, and user notice before mainnet. |
...
| Integrity commitments | Salted private snapshots, opaque subject references, Merkle proofs, public Merkle roots and Stellar transaction hashes. | Private snapshot: medium to high. Public root: designed to be non-identifying. | Supabase database and Stellar public ledger. |
