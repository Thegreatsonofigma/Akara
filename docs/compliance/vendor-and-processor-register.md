...
| Stellar public network and selected Horizon endpoint | Public infrastructure, not treated as a conventional Akara processor. Confirm classification with counsel. | One salted Merkle batch root, Akara anchoring public key, transaction hash, fee and ledger metadata. No names, phone numbers, UUIDs, amounts, currencies, receipts, KYC, payout data, salts, or individual commitments. | Independently timestamp tamper-evident Akara integrity batches. | Globally replicated public ledger. | Protocol use, not a vendor DPA. Horizon operator terms may apply. | Protocol security documentation, endpoint availability, key-management controls, public-data minimisation review. | Public transaction is permanent. Private source snapshots follow Akara retention rules. | Engineering / Privacy Lead. | Medium because public data cannot be deleted. | Complete DPIA and DPCO review before mainnet; use testnet first. |
