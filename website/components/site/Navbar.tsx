    ...
    <header className="fixed inset-x-0 top-0 z-50 bg-black/75 backdrop-blur-xl">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className="inline-flex size-10 items-center justify-center rounded-full border border-hairline text-white md:hidden"
            aria-expanded={open}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            id="mobile-menu"
            className="overflow-hidden border-t border-hairline bg-black/95 backdrop-blur-xl md:hidden"
            ...
            <ul className="flex flex-col gap-1 px-5 py-4">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="block rounded-xl px-4 py-3 text-base text-white/85 transition-colors hover:bg-white/5 hover:text-white"
                    onClick={() => setOpen(false)}
                  >
                    {link.label}
                  </Link>
